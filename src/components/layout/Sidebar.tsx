      ...
      <nav className="px-4 space-y-1">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className="w-5 h-5 transition-colors duration-200 flex-shrink-0 gaming-glow" strokeWidth={2} />
              <span className="font-medium hidden md:block lg:block">{item.label}</span>
              {item.badge && (
                <div className="ml-auto w-5 h-5 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs rounded-full flex items-center justify-center shadow-lg shadow-red-500/50 hidden md:flex lg:flex gaming-pulse">
